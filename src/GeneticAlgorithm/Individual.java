...
public class Individual {//implements Cloneable{
	private class Atom {//implements Clonable{
		...
		public Atom(Vm k, Cloudlet s){
			...
			resource = k;
			task = s;
			Id = s.getCloudletId();//a method in class cloudlet
		//	System.out.println(Id);
		}

		private Atom(Atom old){ // just copying the same old atom
			Id = old.Id;
...
*/
		...
		public double Estimate(){ //time estimate
			Vm tmpVm = GetVM();
	...
	private Individual Cover(Individual old){
		old.GetAtoms().clear();
		for(Atom atom: GetAtoms()){//If there is a list of all atoms: initial item to final item
	...
	public Individual(List<Vm> vmlist, List<Cloudlet> ctlist){ //Each task is given to each virtual machine
...
*/
	...
	public void Show(){
		...
		System.out.println(" ");
	        ...
	        //
	        //< 7 , [3,4,6,7] >
		for(Entry<Integer, ArrayList<Atom>> entry: geneMap.entrySet()){// for( entry: geneMap.entrySet())
			double tmpTime = 0.00;
